...
class GMMTrainer(Trainer):
   ...
   """
    def __init__(self, data_path, model_name="gmm", models_path="models",
    ...
    def ask_config(self, label=""):
        ...
        conf["n_components"] = int(eval(input(
            "Enter number of components in the GMM: ")))
        conf["n_iter"] = int(eval(input(
            "Enter number of iterations: ")))
        conf["n_init"] = int(eval(input(
            "Enter n_init: ")))
        conf["covariance_type"] = str(eval(input(
            "Enter covariance_type (diag or XX): ")))
        return conf
    ...
    def train_model(self, model_obj, trng_data):
        ...
        Args:
            model_obj (GMM): model to be trained
...
class GMMTester(Tester):
    """
    Load a GMM model and predict labels
    """
